...
if( ord(letra) < 32 or ord(letra) > 126 ):
    ...
    sys.exit();
#b/s y ancho
ancho = float(input("Escriba el ancho de banda\n"));
bs=float(input("Esriba los Bits/seg\n"));
periodo=8.0/bs;
frecuencia = 1.0/periodo;

gn = [];
g = [];
nTramas =int ( ancho/frecuencia );# numero de tramas
print ("-per-",periodo,"-frec-",frecuencia,"-tramas-",nTramas);
tt = [];
...
def calc():
    for i in range(1,nTramas+1):
        ...
        teta = calcTeta( an , bn );
        g.append([]);
        for j in np.arange(0.0,periodo,periodo/80):
    ...
    for j in np.arange(0.0,periodo,periodo/80):
        # print (j);
    # for j in np.arange(0.0,8.0+0.1,0.1):
        tt.append(j);
        sumT = 0.0;
...
if n==1:
    ...
    surface["light_color[0]"]    = 0, 0, 0
    surface["light_color[1]"]    = 0, 0, 0
    surface["light_color[2]"]    = 0, 0, 0
    phi, theta = -45, 0
    time = 0
